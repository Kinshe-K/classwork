...
def percentImageError(imageA, imageB):
    ...
    return math.fabs(stat1.median[0] - stat2.median[0])/stat1.median[0]

def main():
    # Add images to testing
    for finger in glob.glob('NISTSpecialDatabase4GrayScaleImagesofFIGS/sd04/png_txt/figs_0/*'):
    ...
    for finger2 in glob.glob('NISTSpecialDatabase4GrayScaleImagesofFIGS/sd04/png_txt/figs_0/*'):
        if finger2.endswith('.txt'):
            None
            #imageDetails_list.append(finger2[-12:])
        else:
            im2 = Image.open(finger2)
            finalTestCheck.append(finger2[-12:])
            test.append(im2)
